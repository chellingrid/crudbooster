<div class='form-group {{$header_group_class}} {{ ($errors->first($name))?"has-error":"" }}' id='form-group-{{$name}}' style="{{@$form['style']}}">
    <label class='col-sm-2 control-label'>{{cbLang($form['label'])}}
        @if($required)
    ...
    <div class="{{$col_width?:'col-sm-10'}}">
        @if($value)
            ...
            ?>
            <p><a data-lightbox='roadtrip' href='{{$url}}'><img style='max-width:160px' title="{{cbLang($form['label'])}}" src='{{$url}}'/></a></p>
            <?php else:?>
        ...
        @endif
        @if(!$value)
            <input type='file' id="{{$name}}" title="{{cbLang($form['label'])}}" {{$required}} {{$readonly}} {{$disabled}} class='form-control' name="{{$name}}"/>
