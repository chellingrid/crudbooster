...
<ul class="nav nav-tabs">
	<li><a href="{{ CRUDBooster::mainpath() }}"><i class='fa fa-file'></i>
			API Documentation</a></li>
	<li><a href="{{ CRUDBooster::mainpath('screet-key') }}"><i
			class='fa fa-key'></i> API Secret Key</a></li>
	<li class='active'><a href="{{ CRUDBooster::mainpath('generator') }}"><i
			class='fa fa-cog'></i> API Generator</a></li>
</ul>
...
.tr-response {
	cursor: pointer
		...
		<form method='post'
			action='{{ route("ApiCustomControllerPostSaveApiCustom")}}'>
			<input type="hidden" name="_token" value="{{ csrf_token() }}" /> <input
				type="hidden" name="id" value="{{$row->id}}">
			<div class='row'>
				<div class='col-sm-8'>
					<div class='form-group'>
						<label>API Name</label> <input type='text' class='form-control'
							value='{{$row->nama}}' required name='nama' id='input-nama' />
					</div>
				</div>

				<div class='col-sm-4'>
					<div class='form-group'>
						<label>Table</label> <select id='combo_tabel' name='tabel'
							required class='form-control'>
							<option value=''>** Choose a Table</option> @foreach($tables as
							$tab)
							<option {{($row->tabel == $tab)?"selected":""}}
								value='{{$tab}}'>{{$tab}}</option> @endforeach
						</select>
					</div>
				</div>
			</div>
			...
			<div class='row'>
					...
					<div class='form-group'>
						...
						<div class='input-group'>
							<span class="input-group-addon" id="basic-addon1"
								style="background: #eeeeee">{{url("api")}}/</span> <input
								type='text' class='form-control' value='{{$row->permalink}}'
								required name='permalink' id='input-permalink' />
						</div>
					</div>
				</div>
				<div class='col-sm-2'>
					<div class='form-group'>
						<label>Action Type</label> <select id='tipe_action' name='aksi'
							required class='form-control'>
							<option value=''>** Selecione Ação</option>
							<option value='list' {{ ($row->aksi == 'list')?"selected":"" }}
								>LISTING</option>
							<option value='detail' {{ ($row->aksi == 'detail')?"selected":""
								}}>DETAIL / READ</option>
							<option value='save_add' {{ ($row->aksi ==
								'save_add')?"selected":"" }}>CREATE / ADD</option>
							<option value='save_edit' {{ ($row->aksi ==
								'save_edit')?"selected":"" }}>UPDATE</option>
							<option value='delete' {{ ($row->aksi == 'delete')?"selected":""
								}}>DELETE</option>
						</select>
					</div>
				</div>
				<div class='col-sm-2'>
					<div class='form-group'>
						<label>Method Type</label> <br /> <label class='radio-inline'> <input
							type='radio' required class='method_type' {{ ($row->method_type
							== 'get')?"checked":"" }} name='method_type' value='get'/> GET
						</label> <label class='radio-inline'> <input type='radio'
							class='method_type' {{ ($row->method_type == 'post')?"checked":""
			...
			<div class='form-group'>
				<div class="clearfix">
					<label><i class='fa fa-cog'></i> Parameters</label> <a
						class='pull-right btn btn-xs btn-primary'
						href='javascript:void(0)' onclick="load_parameters()"><i
						class='fa fa-refresh'></i> Reset</a>
				</div>
				<div class="table-responsive">
					<table id='table-parameters'
						...
						<tfoot style="display: none">
							<tr>
								<td>#</td>
								<td width="20%"><input class='form-control' name='params_name[]'
									type='text' /></td>
								<td width="20%"><select class='form-control'
									name='params_type[]'>
										...
										<optgroup label='Other Validation'>
											...
											<option value='date_format:Y-m-d H:i:s'>DateTime (Y-m-d
												H:i:s)</option>
								...
								<td><input class='form-control' type='text'
									name='params_config[]'></td>
								<td><select class='form-control params_required'
									name='params_required[]'>
										<option value='1'>YES</option>
										<option value='0'>NO</option>
								</select></td>
								<td><select class='form-control params_used'
									name='params_used[]'>
										<option value='1'>YES</option>
										<option value='0'>NO</option>
								</select></td>
								<td class='col-delete'><a class='btn btn-primary'
									href='javascript:void(0)' onclick='addParam()'><i
										class='fa fa-plus'></i></a></td>
							</tr>
						</tfoot>
					</table>
				</div>
				<div class="help-block">To set as comment at description. Add prefix
					* (asterisk) before description. Unless will be set as default
					value.</div>
			</div>
			...
			<div id="response-wrapper"
				...
				<div class='form-group'>
					<div class='clearfix'>
						<label><i class='fa fa-cog'></i> Response</label> <a
							class='pull-right btn btn-xs btn-primary'
							href='javascript:void(0)' onclick='load_response()'><i
							class='fa fa-refresh'></i> Reset</a>
					</div>
						...
						<table id='table-response'
							...
							<tfoot style="display: none">
								<tr class='tr-additional'>
									<td>#</td>
									<td width="20%"><input placeholder='E.g : grand_total'
										name='responses_name[]' class='form-control' type='text' /> <small>Enter
											alias name</small></td>
									<td><select class='form-control' name='responses_type[]'>
											<option value='integer'>Integer</option>
											<option value='boolean'>Boolean</option>
											<option value='string'>String</option>
									...
									<td><input
										...
										<small>Enter sub query without alias name</small></td>
									<td><select class='form-control responses_used'
										name='responses_used[]'>
											<option value='1'>YES</option>
											<option value='0'>NO</option>
									</select></td>
									<td class='col-delete'><a class='btn btn-primary'
										href='javascript:void(0)' onclick='addResponse()'><i
											class='fa fa-plus'></i></a></td>
								</tr>
							</tfoot>
						</table>
