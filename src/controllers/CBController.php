...
class CBController extends Controller
{
    ...
    public function getIndex()
    {
        ...
        $data['page_title'] = cbLang($module->name);
        ...
        foreach ($data['result'] as $row) {
            ...
            foreach ($columns_table as $col) {
                ...
                if (isset($col['image'])) {
                    if ($value == '') {
                        $value = "<a  data-lightbox='roadtrip' rel='group_{{$table}}' title='$title' href='" . asset('vendor/crudbooster/avatar.jpg') . "'><img width='40px' height='40px' src='" . asset('vendor/crudbooster/avatar.jpg') . "'/></a>";
                    } else {
                        $pic = (strpos($value, 'http://') !== false) ? $value : asset($value);
                        $value = "<a data-lightbox='roadtrip'  rel='group_{{$table}}' title='$title' href='" . $pic . "'><img width='40px' height='40px' src='" . $pic . "'/></a>";
                    }
                }
    ...
    public function postExportData()
    {
        ...
        if (Request::input('default_paper_size')) {
            DB::table('cms_settings')->where('name', 'default_paper_size')->update([
                'content' => $papersize
            ]);
        }
    ...
    public function getUpdateSingle()
    {
        ...
        $id = request('id');
        $tablePK = CB::pk($table);
        DB::table($table)->where($tablePK, $id)->update([
            $column => $value
        ]);

        return redirect()->back()->with([
            'message_type' => 'success',
            'message' => cbLang('alert_delete_data_success')
        ]);
    }
    ...
    public function validation($id = null)
    {
        ...
        if ($validator->fails()) {
            ...
            if (Request::ajax()) {
                $res = response()->json([
                    'message' => cbLang('alert_validation_error', [
                        'error' => implode(', ', $message_all)
                    ]),
            ...
            if ($ro['type'] == 'checkbox') {
                ...
                if (is_array($inputdata)) {
                    if ($ro['datatable'] != '') {
                        ...
                        $data_checkbox = DB::table($table_checkbox)->whereIn($table_checkbox_pk, $inputdata)
                            ->pluck($field_checkbox)
                            ->toArray();
    ...
    public function getAdd()
    {
        ...
        if (! CRUDBooster::isCreate() && $this->global_privilege == false || $this->button_add == false) {
            CRUDBooster::insertLog(cbLang('log_try_add', [
                'module' => cbLang(CRUDBooster::getCurrentModule()->name)
            ]));
            CRUDBooster::redirect(CRUDBooster::adminPath(), cbLang("denied_access"));
        }

        $page_title = cbLang("add_data_page_title", [
            'module' => cbLang(CRUDBooster::getCurrentModule()->name)
        ]);
    ...
    public function postAddSave()
    {
        ...
        // insert log
        CRUDBooster::insertLog(cbLang("log_add", [
    ...
    public function getEdit($id)
    {
        ...
        $page_menu = Route::getCurrentRoute()->getActionName();
        $page_title = cbLang("edit_data_page_title", [
            'module' => cbLang(CRUDBooster::getCurrentModule()->name),
            'name' => $row->{$this->title_field}
        ]);
        $command = 'edit';
            ...
            foreach ($this->data_inputan as $ro) {
                ...
                if ($ro['type'] == 'select2') {
                    ...
                    if ($ro['relationship_table'] && $ro["datatable_orig"] != "") {
                        ...
                        if (! isset($params[2]))
                            $params[2] = "id";
                        DB::table($params[0])->where($params[2], $id)->update([
                            $params[1] => implode(",", $inputdata)
                        ]);
                    }
                }
    ...
    public function getDelete($id)
    {
        ...
        // insert log
        CRUDBooster::insertLog(cbLang("log_delete", [
            'name' => $row->{$this->title_field},
            'module' => cbLang(CRUDBooster::getCurrentModule()->name)
        ]));
        ...
        try {
            if (CRUDBooster::isColumnExists($this->table, 'deleted_at')) {
                DB::table($this->table)->where($this->primary_key, $id)->update([
    ...
    public function getDetail($id)
    {
        ...
        $page_menu = Route::getCurrentRoute()->getActionName();
        $page_title = cbLang("detail_data_page_title", [
            'module' => cbLang($module->name),
            'name' => $row->{$this->title_field}
        ]);
        $command = 'detail';
    ...
    public function postDoImportChunk()
    {
        ...
        if (request('file') && request('resume') == 1) {
            ...
            return response()->json([
                'progress' => $prog,
                'last_error' => Cache::get('error_' . $file_md5)
            ]);
        }
    ...
    public function postActionSelected()
    {
        ...
        if ($button_name == 'delete') {
            if (! CRUDBooster::isDelete()) {
                CRUDBooster::insertLog(cbLang("log_try_delete_selected", [
                    'module' => cbLang(CRUDBooster::getCurrentModule()->name)
                ]));
                CRUDBooster::redirect(CRUDBooster::adminPath(), cbLang('denied_access'));
            }
            ...
            try {
                ...
                if (CRUDBooster::isColumnExists($this->table, 'deleted_at')) {
                    ...
                    DB::table($this->table)->whereIn($tablePK, $id_selected)->update([
                        'deleted_at' => date('Y-m-d H:i:s')
                    ]);
                } else {
